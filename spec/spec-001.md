...
- **Real-world usability without full wallet integration.** Alongside the random mock feed, the UI has a manual-entry mode: a real address can be typed in for a transaction the user is actually considering (outbound) or has actually observed (inbound), and screened the same way as any simulated one. This does not cross the architectural boundary above — the tool still never connects to MetaMask, signs, or broadcasts anything. The user reads a real address out of their wallet, types it in here to get a real screening decision, and acts on that decision back in their wallet manually. It's the same "consumes real inputs, gates a decision, never touches the chain" shape the wallet-webhook design already describes, just fed by hand instead of by an event for this POC.
...
15. **Transaction value/amount-based risk signals are out of scope for this POC.** Screening is address-only — the mock provenance score, the sanctions oracle call, and the tier decision all key off the address being screened, never the transaction amount. A real AML program would also screen on value (e.g. structuring/smurfing detection — many transactions kept just under a reporting threshold, unusual value patterns for a given counterparty, etc.), which requires transaction-value data this POC deliberately doesn't collect or reason about. `amount` was removed from the schema and UI entirely (not just hidden) once this became explicit, since keeping it visible implied it factored into the risk decision when it never did.
